...
cat("Mean of price draws:", mean(priors_bayesian$par_draws[, "price"]), "\n")
cat("SD of price draws:", sd(priors_bayesian$par_draws[, "price"]), "\n")

# Plot distribution of one parameter
hist(priors_bayesian$par_draws[, "price"],
...
Test how sensitive your design is to different prior specifications:

```{r}
# Create multiple prior scenarios
scenarios <- list(
  conservative = priors_conservative,
  moderate = priors_fixed,
  optimistic = priors_optimistic
)

# Could use these to create designs and compare efficiency
# (This would be part of a larger design comparison workflow)
```

## Prior Validation

Check that your priors make sense:

```{r}
# Inspect prior structure
priors <- priors_fixed
cat("Attributes with priors:", names(priors$attrs), "\n")

# Check coefficient signs
for (attr in names(priors$attrs)) {
  cat(attr, "- Mean:", priors$attrs[[attr]]$mean, "\n")
}

# Simulate some choice probabilities to check reasonableness
# (This would typically be done with a small design)
```
