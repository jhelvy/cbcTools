...
2. **Create Priors** → `cbc_priors()` - Specify prior preference assumptions (optional)
3. **Generate Design** → `cbc_design()` - Create choice question design
4. **Inspect Design** → `cbc_inspect()` - Evaluate design quality
...
See the [Profiles](profiles.html) article for more details and options on defining profiles, such as including restrictions. 
...
See the [Priors](priors.html) article for more details and options on defining priors. 
...
See the [Design](design.html) article for more details and options on generating experiment designs, such as including "no choice" options, using labeled designs, removing dominant options, and details about each design algorithm.
