...
#'
#' Provides automated suggestions for prior specifications based on attribute
#' scales and desired effect sizes. This function analyzes your profile attributes
#' and generates starting values for priors that can be refined based on domain
#' knowledge.
#'
#' @param profiles A data frame of profiles created by `cbc_profiles()`
#' @param effect_size Character. Desired effect size: `"small"` (0.1-0.3),
#'   `"moderate"` (0.3-1.0, default), or `"large"` (1.0-3.0). Controls the
#'   utility range for continuous attributes.
#' @param categorical_effect Numeric. Effect size increment for categorical
#'   levels. Defaults to `0.1` for small, `0.2` for moderate, and `0.5` for
#'   large effect sizes. Categorical levels are spaced evenly from 0 to
#'   `categorical_effect * (n_levels - 1)`.
#' @param direction Named list mapping attribute names to `"positive"` or
#'   `"negative"` to specify the direction of the effect. Useful for attributes
#'   like price where you expect a negative relationship. Defaults to `NULL`
#'   (positive direction for all attributes).
#'
#' @details
#' This function analyzes each attribute in your profiles and suggests prior
#' parameter values scaled to produce the desired effect size. For continuous
#' attributes, coefficients are scaled so that the full attribute range produces
#' a utility contribution matching the target effect size. For categorical
#' attributes, levels are assigned evenly spaced coefficients with the first
...
#'
#' @return Invisibly returns a named list of suggested prior values. The function
...
cbc_suggest_priors <- function(
  ...
  # Define target utility ranges for each effect size
  target_ranges <- list(
    small = c(0.1, 0.3),
    moderate = c(0.3, 1.0),
    large = c(1.0, 3.0)
  )
  target_range <- target_ranges[[effect_size]]
  target_utility <- mean(target_range)

  # Default categorical effect based on effect size
  if (is.null(categorical_effect)) {
    categorical_effect <- switch(
      effect_size,
      small = 0.1,
      moderate = 0.2,
      large = 0.5
  ...
  for (i in seq_along(profile_attrs)) {
    attr <- profile_attrs[i]
    col_index <- i + 1 # +1 because profileID is column 1

    if (type_ids$continuous[i]) {
      # Continuous attribute
      values <- profiles[[col_index]]
      attr_range <- max(values) - min(values)
      attr_min <- min(values)
      attr_max <- max(values)

      # Suggest coefficient that produces target utility contribution
      suggested_coef <- target_utility / attr_range

      # Check for direction preference
      if (!is.null(direction) && attr %in% names(direction)) {
        if (direction[[attr]] == "negative") {
          suggested_coef <- -suggested_coef
        }
      }

      suggestions[[attr]] <- round(suggested_coef, 6)

      # Create explanation
      utility_contribution <- abs(suggested_coef * attr_range)
      explanations[[attr]] <- sprintf(
        "  %s: Range [%.2f, %.2f] -> suggested %.6f (utility range: \u00B1%.2f)",
        attr,
        attr_min,
        attr_max,
        suggested_coef,
      ...
      levels <- unique(profiles[[col_index]])
      n_levels <- length(levels)

      # For categorical, suggest equal spacing across levels
      # First level will be reference (coefficient = 0)
      level_coefs <- seq(
        0,
        categorical_effect * (n_levels - 1),
        length.out = n_levels
      ...
      # Create named vector
      level_names <- if (is.factor(levels)) {
        as.character(levels(levels))
      } else {
        as.character(levels)
      }

      named_coefs <- stats::setNames(level_coefs, level_names[-1])
      suggestions[[attr]] <- named_coefs

      # Create explanation
      explanations[[attr]] <- sprintf(
        "  %s: %d levels -> suggested c(%s) with '%s' as reference",
        attr,
        n_levels,
        paste(
          sprintf("%s = %.2f", names(named_coefs), named_coefs),
          collapse = ", "
        ),
        level_names[1]
      )
  ...
  cat("\n")
  cat("========================================\n")
  cat("Suggested Priors (", effect_size, " effects)\n", sep = "")
  cat("========================================\n\n")
  ...
  for (i in seq_along(suggestions)) {
    attr <- names(suggestions)[i]
    value <- suggestions[[i]]

    if (is.numeric(value) && length(value) == 1) {
      # Continuous: single value
      cat(sprintf("  %s = %.6f", attr, value))
    } else {
      # Categorical: named vector
      cat(sprintf(
        "  %s = c(%s)",
        attr,
        paste(
          sprintf("'%s' = %.2f", names(value), value),
          collapse = ", "
        )
      ))
    }
  ...
  cat("Notes:\n")
  cat("- These are STARTING suggestions based on attribute scales\n")
  cat("- Adjust based on your domain knowledge and research goals\n")
  cat(
    "- Continuous attributes are scaled to produce ",
    effect_size,
    " effects\n",
    sep = ""
  )
  cat("- For price/cost attributes, consider using negative coefficients\n")
  cat("- The total utility range should stay roughly between -5 and +5\n")
  cat("  to avoid numerical overflow during optimization\n")
  cat("\n")
