...
cbc_decode_design <- function(design) {
    ...
    if (is.null(categorical_structure)) {
        warning(
            "No categorical structure information found. Design may already be decoded or created with an older version."
        )
        return(design)
    }

    # Apply decoding
    decoded_design <- decode_categorical_variables(
        design,
